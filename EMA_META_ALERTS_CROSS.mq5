...
//=========================== BACKEND ==============================//
// Replace <PUBLIC_HOST> with your public backend host (ngrok or deployed URL)
string BASE_URL      = "https://a69a-102-88-115-62.ngrok-free.app/api/status";      
string COMMANDS_URL  = "https://a69a-102-88-115-62.ngrok-free.app/api/commands";    
...
int OnInit()
{
   ...
   AddSymbol(_Symbol); // default chart symbol
...
void CheckCross(SymbolState &s)
{
   ...
   if(CopyBuffer(fastHandle,0,0,2,fast)<2 ||
      CopyBuffer(slowHandle,0,0,2,slow)<2)
   {
...
string TrendDir(string sym,ENUM_TIMEFRAMES tf,int fastMA,int slowMA)
{
   ...
   if(CopyBuffer(fast,0,0,1,f)<1 ||
      CopyBuffer(slow,0,0,1,sma)<1)
   {
   ...
   string result="";
   if(f[0]>sma[0]) result="BULLISH";
   else if(f[0]<sma[0]) result="BEARISH";
...
void CheckBackendCommands()
{
   ...
   if(respCode!=200)
   {
      Print("Command poll failed: ",respCode);
      return;
   }

   string response = CharArrayToString(result);

   if(StringFind(response,"PAUSE")>=0) { IsPaused=true; return; }
   if(StringFind(response,"RESUME")>=0) { IsPaused=false; return; }

   ParseAndExecuteCommands(response);
}

// ------------------ JSON parsing helpers (naive) -----------------
...
void ParseAndExecuteCommands(string json)
{
   ...
   while(true)
   {
      ...
      int objStart = StringFind(json, '{', i);
      if(objStart<0) break;
      ...
      if(type=="" ) { i = objEnd+1; continue; }

      // Execute supported commands
      if(type=="add_pair")
      {
         if(StringLen(symbol)>0) AddSymbol(symbol);
         SendStatusForCommand(id, type, symbol, "ok", "added symbol");
      }
      else if(type=="remove_pair")
      {
         if(StringLen(symbol)>0) RemoveSymbol(symbol);
      ...
      else if(StringFind(type, "toggle_")>=0)
      {
         // toggle_cross, toggle_trend, toggle_volume
         string field = StringSubstr(type, StringFind(type, "toggle_")+7);
         // find pair in Symbols and toggle
         for(int idx=0; idx<SymbolCount; idx++)
         {
            if(Symbols[idx].symbol==symbol)
            {
               if(field=="cross") Symbols[idx].enableCross = !Symbols[idx].enableCross;
               else if(field=="trend") Symbols[idx].enableTrend = !Symbols[idx].enableTrend;
               else if(field=="volume") Symbols[idx].enableVolume = !Symbols[idx].enableVolume;
            }
         }
