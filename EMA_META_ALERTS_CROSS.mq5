...
void SendToBackend(string symbol, string signal, string tf, double price, string msg)
{
   ...
   int respCode = WebRequest(
       ...
       result,
       resHeaders);
   PrintFormat("WebRequest POST -> %s resp=%d headers=%s", BASE_URL, respCode, resHeaders);
   if (ArraySize(result) > 0)
   {
      string respStr = CharArrayToString(result);
      PrintFormat("POST response body: %s", respStr);
   }
}
...
void CheckBackendCommands()
{
   ...
   if (respCode != 200)
   {
      PrintFormat("Command poll failed: %d (url=%s)", respCode, COMMANDS_URL);
      return;
   }

   string response = CharArrayToString(result);
   PrintFormat("Command poll OK, len=%d", StringLen(response));
   ...
   if (StringFind(response, "RESUME") >= 0)
   {
      IsPaused = false;
      return;
   }
...
void SendStatusForCommand(string commandId, string type, string symbol, string resultText, string details)
{
   ...
   if (ArraySize(resp) > 0)
   {
      ...
      PrintFormat("SendStatusForCommand response: %s", respStr);
   }
}
